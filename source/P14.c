#include <stdio.h>
#include <stdlib.h>
#define STUDENTS 3
#define EXAMS 4

int minimum(const int studentGrades[][EXAMS], int pupils, int tests);
int maximum(const int studentGrades[][EXAMS], int pupils, int tests);
double average(const int setOfGrade[], int tests);
void printArray(const int studentGrades[][EXAMS], int pupils, int tests);

int main(void)
{
	int n, student;
	int setOfGrade[EXAMS];
	const int studentGrades[STUDENTS][EXAMS] = { {77, 68, 86, 73}, {96, 87, 89, 78}, {70, 90, 86, 81} };

	printf("The array is:\n");
	printArray(studentGrades, STUDENTS, EXAMS);

	printf("\n\nLowest grade: %d\nHighest grade: %d\n", minimum(studentGrades, STUDENTS, EXAMS), maximum(studentGrades, STUDENTS, EXAMS));
	for (student = 0; student < STUDENTS; student++)
	{
		for (n = 0; n < EXAMS; n++)
			setOfGrade[n] = studentGrades[student][n];
		printf("The average grade for student %d is: %.2f\n", student, average(setOfGrade, EXAMS));
	}

	system("pause");
	return 0;
}

void printArray(const int studentGrades[][EXAMS], int pupils, int tests)
{
	int i, j;

	printf("                 [0]  [1]  [2]  [3]");
	for (i = 0; i < pupils; i++)
	{
		printf("\nstudentGrades[%d] ", i);
		for (j = 0; j < tests; j++)
			printf("%-5d", studentGrades[i][j]);
	}
}

int minimum(const int studentGrades[][EXAMS], int pupils, int tests)
{
	int i, j;
	int lowGrade = 100;

	for (i = 0; i < pupils; i++)
		for (j = 0; j < tests; j++)
			if (studentGrades[i][j] < lowGrade)
				lowGrade = studentGrades[i][j];

	return lowGrade;
}

int maximum(const int studentGrades[][EXAMS], int pupils, int tests)
{
	int i, j;
	int highGrade = 0;

	for (i = 0; i < pupils; i++)
		for (j = 0; j < tests; j++)
			if (studentGrades[i][j] > highGrade)
				highGrade = studentGrades[i][j];

	return highGrade;
}

double average(const int setOfGrade[], int tests)
{
	int i;
	int total = 0;

	for (i = 0; i <= tests; i++)
		total += setOfGrade[i];

	return (double)total / tests;
}